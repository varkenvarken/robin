...
class environment:
    ...
    def op2(self, r2, r1, r0, addr):  # alu
        ...
        ops = {
            ...
            12: lambda x, y, c: x << y,
            13: lambda x, y, c: x >> y,
            14: lambda x, y, c: (x * y) & 0xffffffff,
            15: lambda x, y, c: (x * y) >> 32,
            16: lambda x, y, c: x // y,
            17: lambda x, y, c: (-1 if ((x ^ y) & 0x80000000) else 1) * (absu(x) // absu(y)),
            18: lambda x, y, c: x - y * (x // y),
            19: lambda x, y, c: (-1 if ((x ^ y) & 0x80000000) else 1) * (absu(x) - absu(y) * (absu(x) // absu(y))),
        }
